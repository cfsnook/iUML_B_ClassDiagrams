...
public abstract class ClassdiagramsAbstractExpression {
	...
	protected void setStatus(int severity, String message, Throwable throwable) {
		...
		if (!this.status.isOK()) {
			ClassdiagramsDiagramEditorPlugin.getInstance().logError("Expression problem:" + message + "body:" + body(), //$NON-NLS-1$//$NON-NLS-2$
					throwable);
		}
	}
	...
	public Object evaluate(Object context, Map env) {
		if (context().isInstance(context)) {
			try {
				...
				ClassdiagramsDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), e); //$NON-NLS-1$
			}
		}
		return null;
	}
